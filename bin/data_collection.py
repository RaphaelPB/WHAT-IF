...
class Database():   
    ...
    def harvest_sheet(self,Path,SheetName,Header=5,Index=1,DataType=0,Scenario=None,MultiIndexName=None,MatrixName=None,ColIndexName=None,OnlyCols=None):
        #Harvests one excel sheet, starting at row "Header"
        Data = pd.read_excel(Path,sheet_name=SheetName, skiprows=Header, index_col=list(range(Index)), usecols=OnlyCols)
        ...
        if DataType==2: #Matrix data
            ...
            if ColIndexName != None and ColIndexName != 'nan':
                self.val[ColIndexName]=[k for k in Data.axes[1]]
    ...
    def harvest_all(self,Paths):
        ...
        for path in Paths:
            ...
            for k in self.info['nsheet']:
                ...
                if self.update==1 and self.info['Update'][k]==1:
                    warning= 'UPDATE ONLY: Sheet "' + self.info['SheetName'][k] +'" was updated'
                    print(warning)
